...
public class KSBankEncSocket
{
	...
	{
		return KSBankMsg.s2b(new StringBuffer().append(System.currentTimeMillis()).append(Long.MAX_VALUE).substring(0,16));
	}
	...
	public static byte[] kscms_encrypt(byte[] p_kbuf, byte[] p_dbuf)
	{
		...
		{
			System.out.println("ERROR: EncrpyMsg ("+p_dbuf==null?"null":KSBankMsg.b2s(p_dbuf) +")");
			e.printStackTrace();
		}
	...
	public static byte[] kscms_sndrcv_socket(String ipaddr, int port, byte[] p_dbuf, int timeout)
	{
		try
		{
			byte[] p_kbuf	= KSBankMsg.s2b(new StringBuffer().append(System.currentTimeMillis()).append(Long.MAX_VALUE).substring(0,16));
			byte[] e_kbuf	= ks_rsa_encrypt(p_kbuf);
		...
		{
			System.out.println("ERROR: kscms_sndrcv_socket 통신오류발생 ("+ipaddr+","+port+","+p_dbuf==null?"null":KSBankMsg.b2s(p_dbuf) +")");
			ie.printStackTrace();
			return null;
		}catch(Exception e)
		{
			System.out.println("ERROR: kscms_sndrcv_socket 기타오류발생 ("+ipaddr+","+port+","+p_dbuf==null?"null":KSBankMsg.b2s(p_dbuf) +")");
			e.printStackTrace();
			return null;
	...
	public static byte[] sndrcv_socket(String ipaddr, int port, byte[] sbuf) throws IOException
	{
		...
		try
		{
			...
			/* System.out.println("INFO: sndrcv_socket ("+read_len+")byte 수신 length_len=("+KSBankMsg.b2s(rbuf,0,4) +")"); */
...
*/
	...
	public static byte[] rcv_socket(Socket sock) throws IOException
	{
		DataInputStream		din	= null;
			...
			/* System.out.println("INFO: rcv_socket ("+read_len+")byte 수신 length_len=("+KSBankMsg.b2s(rbuf,0,4) +")"); */
			if (read_len == 0) return null; // socket closed
