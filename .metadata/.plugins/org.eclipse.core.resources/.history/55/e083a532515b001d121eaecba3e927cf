...
import Crypto.KSBankMsg;
import Util.CUtil;
import Util.DUtil;
import Util.LUtil;
import Util.SUtil;

public class TestFirmBypass extends Thread{
	...
	public void run()
	{
		...
		try {
			...
			socket	= new Socket(ksnet_ip, ksnet_port);
			...
			byte[] snd_buf = KSBankMsg.s2b("6000"+auth_key+test_send);
